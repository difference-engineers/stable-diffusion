...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
                 ...
                 v_posterior=0.,
                 # weight for choosing posterior variance as sigma = (1-v) *
                 # beta_tilde + v * beta
                 l_simple_weight=1.,
    ...
    def register_schedule(
        ...
        posterior_variance = (1 - self.v_posterior) * betas * (
            1. - alphas_cumprod_prev) / (1. - alphas_cumprod) + self.v_posterior * betas
        ...
        if self.parameterization == "eps":
            ...
            lvlb_weights = 0.5 * \
                np.sqrt(torch.Tensor(alphas_cumprod)) / (2. * 1 - torch.Tensor(alphas_cumprod))
        else:
        ...
        if hasattr(self, "split_input_params"):
            if self.split_input_params["patch_distributed_vq"]:
                ...
                if isinstance(self.first_stage_model, VQModelInterface):
                    ...
                    output_list = [self.first_stage_model.decode(
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if self.cond_stage_key in [
                ...
                # assuming padding of unfold is always 0 and its dilation is
                # always 1
                n_patches_per_row = int((w - ks[0]) / stride[0] + 1)
                full_img_h, full_img_w = self.split_input_params['original_image_size']
                # as we are operating on latents, we need the factor from the original image size to the
                # spatial latent size to properly rescale the crops for
                # regenerating the bbox annotations
                num_downs = self.first_stage_model.encoder.num_resolutions - 1
                rescale_latent = 2 ** (num_downs)

                # get top left postions of patches as conforming for the bbbox tokenizer, therefore we
                # need to rescale the tl patch coordinates to be in between
                # (0,1)
                tl_patch_coordinates = [(rescale_latent * stride[0] * (patch_nr % n_patches_per_row) / full_img_w,
                                         rescale_latent * stride[1] * (patch_nr // n_patches_per_row) / full_img_h)
                                        for patch_nr in range(z.shape[-1])]

                # patch_limits are tl_coord, width and height coordinates as
                # (x_tl, y_tl, h, w)
                patch_limits = [
                    (x_tl,
                     y_tl,
                     ...
                     ks[0] /
                        ...
                        ks[1] /
                        full_img_h) for x_tl,
                    y_tl in tl_patch_coordinates]
                # patch_values = [(np.arange(x_tl,min(x_tl+ks, 1.)),np.arange(y_tl,min(y_tl+ks, 1.))) for x_tl, y_tl in tl_patch_coordinates]

                # tokenize crop coordinates for the bounding boxes of the
                # respective patches
                patch_limits_tknzd = [torch.LongTensor(self.bbox_tokenizer._crop_encoder(bbox))[None].to(
                    self.device) for bbox in patch_limits]  # list of length l with tensors of shape (1, 2)
    ...
    def log_images(
        ...
        if plot_progressive_rows:
            with self.ema_scope("Plotting Progressives"):
                img, progressives = self.progressive_denoising(c, shape=(
                    self.channels, self.image_size, self.image_size), batch_size=N)
