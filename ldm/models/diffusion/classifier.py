...
class NoisyLatentImageClassifier(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.label_key = label_key if not hasattr(
            self.diffusion_model,
            'cond_stage_key') else self.diffusion_model.cond_stage_key
    ...
    def compute_top_k(self, logits, labels, k, reduction="mean"):
        ...
        if reduction == "mean":
            return (top_ks == labels[:, None]).float().sum(
                dim=-1).mean().item()
        elif reduction == "none":
    ...
    def shared_step(self, batch, t=None):
        x, * \
            _ = self.diffusion_model.get_input(batch, k=self.diffusion_model.first_stage_key)
    ...
    def reset_noise_accs(self):
        self.noisy_acc = {t: {'acc@1': [],
                              'acc@5': []} for t in range(0,
                                                          self.diffusion_model.num_timesteps,
