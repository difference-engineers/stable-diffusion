...
def analytic_kernel(k):
    ...
    # Crop the edges of the big kernel to ignore very small values and
    # increase run time of SR
    crop = k_size // 2
...
def gen_kernel(k_size=np.array([15, 15]), scale_factor=np.array(
        [4, 4]), min_var=0.6, max_var=10., noise_level=0):
    """"
...
def add_resize(img, sf=4):
    ...
    if rnum > 0.8:  # up
        ...
        sf1 = 1.0
    img = cv2.resize(img, (int(sf1 *
                               img.shape[1]), int(sf1 *
                                                  img.shape[0])), interpolation=random.choice([1, 2, 3]))
    ...
    for i in shuffle_order:
        ...
        if i == 0:
            ...
            if random.random() < 0.75:
                ...
                img = cv2.resize(img,
                                 (int(1 / sf1 * img.shape[1]),
                                  int(1 / sf1 * img.shape[0])),
                                 interpolation=random.choice([1,
                                                              2,
                                                              3]))
            else:
    ...
    if sf == 4 and random.random() < scale2_prob:  # downsample1
        if np.random.rand() < 0.5:
            image = cv2.resize(image,
                               (int(1 / 2 * image.shape[1]),
                                int(1 / 2 * image.shape[0])),
                               interpolation=random.choice([1,
                                                            2,
                                                            3]))
        else:
...
# TODO incase there is a pickle error one needs to replace a += x with a =
# a + x in add_speckle_noise etc...
def degradation_bsrgan_plus(
    ...
    img = cv2.resize(img, (int(1 /
                               sf *
                               hq.shape[1]), int(1 /
                                                 sf *
                                                 hq.shape[0])), interpolation=random.choice([1, 2, 3]))
...
if __name__ == '__main__':
    ...
    for i in range(20):
        ...
        lq_bicubic_nearest = cv2.resize(util.single2uint(img_lq_bicubic), (int(
            sf * img_lq.shape[1]), int(sf * img_lq.shape[0])), interpolation=0)
